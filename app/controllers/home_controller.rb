...
class HomeController < ApplicationController
  # GET /
  def index
    return unless is_session_valid?
  ...
  def get_user(email, password)
    Auth.find_by(email: email, password: password)
  end

  def new_user?(email)
    return true if Auth.where(email: email).blank?

    false
  end

  def correct_user?(email, password)
    return true if Auth.where(email: email, password: password).present?

    false
  end

  def user_exists?(email)
    return true if Auth.where(email: email).present?
  ...
  def create_user(params)
    puts(params)
    user = Auth.create(name: params[:name], email: params[:email], password: params[:password],
                       user_type: params[:type], is_valid: true)
